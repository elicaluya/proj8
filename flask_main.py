...
def select():
    ...
    for cal in calendars:
        ...
        for event in data['items']:
            ...
            if 'dateTime' in event['start']:
                ...
                # Adding dates and start and end times to their respective list
                date_list.append(e_start.date())
                time_list.append(start_time)
                time_list.append(end_time)
                
                # Compares times to see if the event is in the time range or not
                if end_time > range_start and start_time < range_end:
                    flask.flash("EVENT FOUND: " + event['summary'] + " on " + str(start_date) + " from " + str(start_time) + " to " + str(end_time))
                else:
                    flask.flash("Event found on " + str(start_date) + " but not in time range")
            else:
                flask.flash("ALL DAY EVENT FOUND: " + event['summary'] + " at " + str(event['start']['date']))
                flask.flash("No free time because all day event")
    ...
    for i in range(len(date_list)-1):
        ...
        if time_list[i*2] < range_end and time_list[(i*2)+1] > range_start:
            ...
            if date_list[i] == date_list[i+1] and time_list[i*2] <= range_start and time_list[(i*2)+2] <= range_end and time_list[(i*2)+3] >= range_end:
                flask.flash("FREE TIME on " + str(date_list[i]) + " ==> " + str(time_list[(i*2)+1]) + " to " + str(time_list[(i*2)+2]))
             
            # For more than one event in the day and there is free time between and after last event    
            elif date_list[i] == date_list[i+1] and time_list[i*2] <= range_start and time_list[(i*2)+2] < range_end and time_list[(i*2)+3] < range_end:
                flask.flash("FREE TIME on " + str(date_list[i]) + " ==> " + str(time_list[(i*2)+1]) + " to " + str(time_list[(i*2)+2]))
                flask.flash("FREE TIME on " + str(date_list[i]) + " ==> " + str(time_list[(i*2)+3]) + " to " + str(range_end))
            
            # For more than one event in the day and there is free time before first event and between events    
            elif date_list[i] == date_list[i+1] and time_list[i*2] > range_start and time_list[(i*2)+2] <= range_end and time_list[(i*2)+1] < range_end:
                flask.flash("FREE TIME on " + str(date_list[i]) + " ==> " + str(range_start) + " to " + str(time_list[i*2]))
                flask.flash("FREE TIME on " + str(date_list[i]) + " ==> " + str(time_list[(i*2)+1]) + " to " + str(time_list[(i*2)+2]))
            
            # For more than one event and there is free time between events, before first event, and after last event  
            elif date_list[i] == date_list[i+1] and time_list[i*2] > range_start and time_list[(i*2)+1] < range_end and time_list[(i*2)+3] < range_end:
                flask.flash("FREE TIME on " + str(date_list[i]) + " ==> " + str(range_start) + " to " + str(time_list[i*2]))
                flask.flash("FREE TIME on " + str(date_list[i]) + " ==> " + str(time_list[(i*2)+1]) + " to " + str(time_list[(i*2)+2]))
            
            # For one event in a day and there is free time before and after the event         
            elif time_list[i*2] > range_start and time_list[(i*2)+1] < range_end:
                flask.flash("FREE TIME on " + str(date_list[i]) + " ==> " + str(range_start) + " to " + str(time_list[i*2]))
                flask.flash("FREE TIME on " + str(date_list[i]) + " ==> " + str(time_list[(i*2)+1]) + " to " + str(range_end))
            
            # For one event in a day and there is free time after the event
            elif time_list[i*2] <= range_start and time_list[(i*2)+1] < range_end:
                flask.flash("FREE TIME on " + str(date_list[i]) + " ==> " + str(time_list[(i*2)+1]) + " to " + str(range_end))
            
            # For one event in a day and there is free time before the event
            elif time_list[i*2] > range_start and time_list[(i*2)+1] >= range_end:
                flask.flash("FREE TIME on " + str(date_list[i]) + " ==> " + str(range_start) + " to " + str(time_list[i*2]))
